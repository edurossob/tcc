...
----------
- class Dribble: implements an OpenAI custom gym
...
class Dribble(gym.Env):
    def __init__(self, ip, server_p, monitor_p, r_type, enable_draw) -> None:
        ...
        # memory
        me = self.player.world.robot.loc_head_position[:2]
        b = self.player.world.ball_abs_pos[:2]
        self.last_b_x = self.player.world.ball_abs_pos[0]
        self.last_b_y = self.player.world.ball_abs_pos[1]
        self.last_b_dist = np.linalg.norm(b-me)
    ...
    def reset(self, seed=0):
        ...
        for _ in range(25): 
            self.player.scom.unofficial_beam((-1,0,0.50),0) # beam player continuously (floating above ground)
    ...
    def step(self, action):
        ...
        me_x = me[0]
        b = w.ball_abs_pos[:2]
        b_x = w.ball_abs_pos[0]
        b_y = w.ball_abs_pos[1]
        # b_rel = w.ball_rel_torso_cart_pos[:2]
        b_dist = np.linalg.norm(b-me)

        
        delta_b_dist = np.linalg.norm(b_dist - self.last_b_dist) 
        delta_b_x = np.linalg.norm(self.last_b_x - b_x)
        delta_b_y = np.linalg.norm(self.last_b_y - b_y)
        # reward = bola_agente + 0.05e^(-d_bola_agente) - opp_agent + 5 xBall
        # + 10 agent_completed_dribble - 10 opp_completed_dribble - 1 agent_fell
        # reward = delta_b_dist + 0.05 * math.exp(-b_dist) + 5*delta_b_x
        # reward = 0.05 * math.exp(-b_dist) + 3*delta_b_x
        reward = 3*delta_b_x - delta_b_y


        lost_ball = (me_x - b_x) > 1

        # terminal state: the robot is falling or timeout
        terminal = r.cheat_abs_pos[2] < 0.3 or self.step_counter > 300 or lost_ball


        # memory
        self.last_b_x = b_x
        self.last_b_y = b_y
        self.last_b_dist = b_dist
...
# Train start:    11/11/2024 16:39:24
# Train end:      12/11/2024 00:28:37
# Train duration: 7:49:12