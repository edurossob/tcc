...
----------
- class Simple_Run: implements an OpenAI custom gym
- class Train:  implements algorithms to train a new model or test  an existing model
'''

class Simple_Run(gym.Env):
...
class Train(Train_Base):
    ...
    def train(self, args):
        ...
        learning_rate = 3e-4
        folder_name = f'Simple_Run_R{self.robot_type}'
        ...
        def init_env(i_env):
            def thunk():
                return Simple_Run( self.ip , self.server_p + i_env, self.monitor_p_1000 + i_env, self.robot_type, False )
            return thunk
    ...
    def test(self, args):
        ...
        env = Simple_Run( self.ip, self.server_p-1, self.monitor_p, self.robot_type, True )
...
# Train start:    03/10/2024 17:56:32
# Train end:      04/10/2024 02:28:00
# Train duration: 8:31:28